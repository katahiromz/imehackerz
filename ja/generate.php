...
<div class="header">
	...
	<ul class="menu clearFix">
		...
		<li><a href="interfaces.html">インターフェース</a></li>
...
function get_body($entry_type) {
	...
	(
		...
		(
			($entry_type == "構造体" || $entry_type == "インターフェース") ?
			'<h2>メンバー</h2>' . "\n" :
			'<h2>引数</h2>' . "\n"
		) . 
		...
		'<tr>' . "\n" .
			'<th>ヘッダー</th>' . "\n" .
			'<td>' . (
				get_field('header') != '' ? get_field('header') : "(不明)"
			) . "</td>\n" .
		'</tr>' . "\n" .
		'<tr>' . "\n" .
			'<th>モジュール</th>' . "\n" .
			'<td>' . (
				get_field('modules') != '' ? get_field('modules') : "(不明)"
			) . "</td>\n" .
		'</tr>' . "\n" .
		'</table>' . "\n" : ""
	) . (
		...
		'<p>' . (
			get_field("prev_page") != '' ?
			'<a href="' . entry_name_to_file_name(get_field("prev_page")) . '.html">&lt;&lt;前のページ</a> ' :
			""
		) . (
			get_field("next_page") != '' ?
			'<a href="' . entry_name_to_file_name(get_field("next_page")) . '.html">次のページ&gt;&gt;</a> ' :
			""
		) . '</p>' : ""
	);
}
	...
	if ($result) {
		...
		if ($fp) {
			...
			foreach ($arrays as $assoc) {
				$entry_name = $assoc['entry_name'];
				$entry_type = $assoc['entry_type'];
				if ($entry_type != constant('TYPE_STRUCTURE')) continue;
				fputs($fp, '<li><a href="' . entry_name_to_file_name($entry_name) . '.html">' . $entry_name . " " . $entry_type . '</a></li>' . "\n");
			}
			fputs($fp, 
				"</ul>\n"
		...
		if ($fp) {
			$header = get_header("インターフェース", "interfaces");
			fputs($fp, $header);
			fputs($fp, 
				"<h2>インターフェース</h2>\n" .
				"<ul>\n"
			...
			foreach ($arrays as $assoc) {
				$entry_name = $assoc['entry_name'];
				$entry_type = $assoc['entry_type'];
				if ($entry_type != constant('TYPE_INTERFACE')) continue;
				fputs($fp, '<li><a href="' . entry_name_to_file_name($entry_name) . '.html">' . $entry_name . " " . $entry_type . '</a></li>' . "\n");
			}
			fputs($fp, 
				"</ul>\n"
			);
			$footer = get_footer('interfaces');
			fputs($fp, $footer);
			fclose($fp);
		}
